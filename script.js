/* ================================================================
   ...
   Fitur:
     ...
     9. AJAX booking save to database
...
btnPreview?.addEventListener('click', () => {
  ...
  // Simpan booking ke database via AJAX
...
/* ── 10. Save booking to database (AJAX) ────────────────────── */
function saveBookingToDatabase(data) {
  const formData = new FormData();
  formData.append('nama', data.nama);
  formData.append('no_hp', data.noHp);
  formData.append('mobil_nama', selectedCar.name);
  formData.append('mobil_harga', selectedCar.price);
  formData.append('tgl_mulai', data.tglMulai);
  formData.append('jam_mulai', data.jamMulai);
  formData.append('tgl_selesai', data.tglSelesai);
  formData.append('jam_selesai', data.jamSelesai);
  formData.append('opsi_sewa', data.opsi);
  formData.append('lokasi', data.lokasi || '');
  formData.append('catatan', data.catatan || '');

  fetch('prosesbooking.php', {
    method: 'POST',
    body: formData
  })
  ...
  .then(result => {
    if (result.success) {
      console.log('✅ Booking berhasil disimpan, ID:', result.booking_id);
    } else {
      console.warn('⚠️ Gagal menyimpan booking:', result.message);
    }
  })
  .catch(error => {
    console.warn('⚠️ Error saat menyimpan booking:', error);
  });
}
